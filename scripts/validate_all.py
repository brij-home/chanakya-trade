...
def main():
    ...
    pytest_exe = os.path.join(root_dir, ".venv", "Scripts", "pytest.exe") if os.name == 'nt' else os.path.join(root_dir, ".venv", "bin", "pytest")

    steps = [
        ("1. React Hook Invariant AST Linter", "node scripts/audit-react-hooks.js", macos_app_dir),
        ("2. Frontend Vitest Component & Store Tests", "cmd /c npm test" if os.name == 'nt' else "npm test", macos_app_dir),
        ("3. Production Web Bundle Build", "cmd /c npm run build:web" if os.name == 'nt' else "npm run build:web", macos_app_dir),
        ("4. Backend Core API & SSE Test Suites", f'"{pytest_exe}" -v tests/test_api_broker.py tests/test_sse_streaming.py', root_dir),
    ]
