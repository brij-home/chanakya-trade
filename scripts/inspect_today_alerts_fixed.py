...
def main():
    ...
    opt_alerts = [
        a
        for a in today_alerts
        if "OPTION" in a.get("alert_type", "") or "GAMMA" in a.get("alert_type", "")
    ]
    for a in opt_alerts[:5]:
        print(
            f"ID: {a.get('alert_id')} | Symbol: {a.get('symbol')} | Type: {a.get('alert_type')} | Dir: {a.get('direction')}"
        )
        print(
            f"  Strike: {a.get('strike')} | OptType: {a.get('option_type')} | Contract: {a.get('contract_symbol')}"
        )
        print(
    ...
    eq_alerts = [
        a
        for a in today_alerts
        if "OPTION" not in a.get("alert_type", "") and "GAMMA" not in a.get("alert_type", "")
    ]
    for a in eq_alerts:
        print(
            f"Time: {a.get('timestamp')} | ID: {a.get('alert_id')} | Symbol: {a.get('symbol')} | Type: {a.get('alert_type')} | Dir: {a.get('direction')}"
        )
        print(
            f"  LTP: {a.get('ltp')} | Trigger: {a.get('trigger_level')} | Target: {a.get('target_level')} | SL: {a.get('stop_loss')}"
        )
        print(
            f"  Status: {a.get('target_status')} | Milestones: {a.get('achieved_milestones')} | Invalidated: {a.get('is_invalidated')} ({a.get('invalidation_reason')})"
        )
