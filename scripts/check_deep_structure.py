...
for sym in symbols:
    ...
    print(f"52-Week High: {df['High'].tail(252).max():.2f}, 52-Week Low: {df['Low'].tail(252).min():.2f}")
    
    # Recent 10 daily candles
    print("\nRecent 10 trading sessions:")
    for idx, row in df.tail(10).iterrows():
        print(f"{idx.strftime('%Y-%m-%d')} | O: {row['Open']:8.2f} | H: {row['High']:8.2f} | L: {row['Low']:8.2f} | C: {row['Close']:8.2f} | Vol: {int(row['Volume']):10d}")
    ...
    for i in range(2, len(recent) - 2):
        if recent['High'].iloc[i] > recent['High'].iloc[i-1] and recent['High'].iloc[i] > recent['High'].iloc[i-2] and \
           recent['High'].iloc[i] > recent['High'].iloc[i+1] and recent['High'].iloc[i] > recent['High'].iloc[i+2]:
            swing_highs.append((recent.index[i].strftime('%Y-%m-%d'), recent['High'].iloc[i]))
        if recent['Low'].iloc[i] < recent['Low'].iloc[i-1] and recent['Low'].iloc[i] < recent['Low'].iloc[i-2] and \
           recent['Low'].iloc[i] < recent['Low'].iloc[i+1] and recent['Low'].iloc[i] < recent['Low'].iloc[i+2]:
    ...
    print(f"  Trailing P/E: {inf.get('trailingPE', 'N/A')}, Forward P/E: {inf.get('forwardPE', 'N/A')}")
