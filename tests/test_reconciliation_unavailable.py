...
def test_reconciliation_without_broker_returns_unavailable(client):
    """
    When no execution broker session is available,
    ...
    to itself.
    """
    with patch("brokers.session.get_execution_broker", return_value=None):
...
def test_reconciliation_broker_exception_returns_unavailable(client):
    """
    When execution-broker resolution raises an exception, the endpoint must still
    return status='UNAVAILABLE' rather than a self-comparison report.
    """
    with patch("brokers.session.get_execution_broker", side_effect=RuntimeError("session expired")):
        resp = client.get("/api/reconciliation")
...
def test_reconciliation_never_compares_ledger_to_itself(client):
    ...
    # A broker snapshot alone is still insufficient.  The application must not
    # substitute broker portfolio data for an independent local ledger.
    assert data.get("status") == "UNAVAILABLE"
    assert "baseline" in data.get("reason", "").lower()

    # Even an unavailable result includes the actual broker snapshot provenance.
