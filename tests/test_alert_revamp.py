...
def test_fno_stock_routing_fallback(monkeypatch):
    ...
    assert chat_id == "-1004393392375", (
        f"Expected dedicated FnO channel (-1004393392375), got {chat_id}"
    )
...
def test_milestone_alert_preserves_original_call_time():
    ...
    ms_data = MilestoneAlertData.from_alert(
        alert=alert, milestone_type="TARGET_1", timestamp="2026-09-11 10:15:00 IST"
    )
...
def test_currency_pair_cmp_formatting():
    ...
    assert (
        "Spot CMP: ₹86.12" not in rendered
    )  # Must not truncate to 2 decimals or use generic Spot CMP
