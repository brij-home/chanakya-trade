...
class TestOAuthLocalServer:
    def test_captures_fyers_auth_code(self):
        ...
        result = _oauth_local_server(port, "/fyers/callback", "auth_code", timeout=1.5)
    ...
    def test_captures_zerodha_request_token(self):
        ...
        threading.Thread(target=_hit, daemon=True).start()
        result = _oauth_local_server(port, "/zerodha/callback", "request_token", timeout=1.5)
    ...
    def test_captures_upstox_code(self):
        ...
        threading.Thread(target=_hit, daemon=True).start()
        result = _oauth_local_server(port, "/upstox/callback", "code", timeout=1.5)
    ...
    def test_success_page_returned_to_browser(self):
        ...
        _oauth_local_server(port, "/fyers/callback", "auth_code", timeout=1.5)
    ...
    def test_returns_none_when_port_busy(self):
        ...
        try:
            result = _oauth_local_server(port, "/fyers/callback", "auth_code", timeout=0.5)
    ...
    def test_returns_none_on_timeout(self):
        ...
        port = _free_port()
        result = _oauth_local_server(port, "/fyers/callback", "auth_code", timeout=0.3)
        assert result is None
    ...
    def test_captures_multiple_params(self):
        ...
        threading.Thread(target=_hit, daemon=True).start()
        result = _oauth_local_server(port, "/cb", "code", "state", timeout=1.5)
    ...
    def test_missing_param_returns_empty_string(self):
        ...
        threading.Thread(target=_hit, daemon=True).start()
        result = _oauth_local_server(port, "/cb", "auth_code", "state", timeout=1.5)
