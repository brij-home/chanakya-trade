...
export default function CommandPalette({ isOpen, onClose, onOpenOrderTicket }) {
  ...
  const quickActions = [
    ...
    { label: `🚀 Breakout Council Consensus (${activeSymbol || 'RELIANCE'})`, cmd: `council breakout ${activeSymbol || 'RELIANCE'}`, icon: '🏛️', cat: 'Councils' },
    { label: `🎯 Options Sniper Council (${activeSymbol || 'NIFTY'})`, cmd: `council options_sniper ${activeSymbol || 'NIFTY'}`, icon: '🎯', cat: 'Councils' },
    { label: `💎 Multibagger Compounder Council (${activeSymbol || 'TATAMOTORS'})`, cmd: `council multibagger ${activeSymbol || 'TATAMOTORS'}`, icon: '💎', cat: 'Councils' },
    { label: `🌐 Macro Regime Council (${activeSymbol || 'HDFCBANK'})`, cmd: `council macro_regime ${activeSymbol || 'HDFCBANK'}`, icon: '🌐', cat: 'Councils' },
    { label: `🏛️ Core Value Council (${activeSymbol || 'INFY'})`, cmd: `council core_value ${activeSymbol || 'INFY'}`, icon: '🏛️', cat: 'Councils' },
    { label: `🧠 Mark Minervini SEPA/VCP Analysis (${activeSymbol || 'RELIANCE'})`, cmd: `persona minervini ${activeSymbol || 'RELIANCE'}`, icon: '🚀', cat: 'Personas' },
    { label: `🧠 Vijay Kedia SMILE Framework (${activeSymbol || 'POLYCAB'})`, cmd: `persona kedia ${activeSymbol || 'POLYCAB'}`, icon: '💎', cat: 'Personas' },
    { label: `🧠 Nassim Taleb Convexity & Tail Risk (${activeSymbol || 'NIFTY'})`, cmd: `persona taleb ${activeSymbol || 'NIFTY'}`, icon: '🛡️', cat: 'Personas' },
    { label: `🧠 Richard Wyckoff VSA Springs (${activeSymbol || 'RELIANCE'})`, cmd: `persona wyckoff ${activeSymbol || 'RELIANCE'}`, icon: '📈', cat: 'Personas' },
    { label: `🧠 Smart Money Concepts / ICT Liquidity (${activeSymbol || 'BANKNIFTY'})`, cmd: `persona smc ${activeSymbol || 'BANKNIFTY'}`, icon: '🎯', cat: 'Personas' },
    { label: `🛡️ Defined-Risk Spread Builder (${activeSymbol || 'NIFTY'} Bull Call Spread)`, cmd: `spreads ${activeSymbol || 'NIFTY'} BULL_CALL_SPREAD`, icon: '🛡️', cat: 'Options' },
    { label: `📊 Capital Gains Tax Estimator (STCG / LTCG)`, cmd: `tax 50000 90`, icon: '🧾', cat: 'Tax & Cost' },
    { label: `🌾 Tax-Loss Harvesting Opportunity Scanner`, cmd: `harvest`, icon: '🌾', cat: 'Tax & Cost' },
