...
def parse_binary_frame(data: bytes, token_map: Optional[dict[str, str]] = None) -> list[MStockTick]:
    ...
    - Exactly 379 bytes or multiple 379-byte packets
    - Exactly 51 bytes or multiple 51-byte LTP packets
    ...
    if len(data) > 2:
        try:
            ...
            if pkt_count > 0 and (2 + pkt_count * 379 == len(data)):
                offset = 2
        except Exception:
            offset = 0
    ...
    # Case C: Single 379-byte packet anywhere
    if remaining >= 379:
    ...
    # Case D: Single 51-byte packet anywhere
    if remaining >= 51:
