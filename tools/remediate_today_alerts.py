...
source_path = (
    ...
    if (
        bak_path.exists()
        and bak_path.stat().st_size > (file_path.stat().st_size if file_path.exists() else 0)
    )
    else file_path
)
...
for a in data:
    ...
    if is_opt:
        ...
        sl_prem = float(
            a.get("stop_loss") or (opt_plan.get("sl_premium") if opt_plan else 0.0) or 0.0
        )
        t1_prem = float(
            a.get("target_level") or (opt_plan.get("t1_premium") if opt_plan else 0.0) or 0.0
        )
        ...
        plan["target_moonshot"] = (
            f"₹{t3_prem:,.2f}" if t3_prem else f"₹{round(opt_ltp * 4.0, 2):,.2f}"
        )
        if sl_prem:
        ...
        if not plan.get("profit_rule"):
            plan["profit_rule"] = (
                f"Book 50% at T1 ({plan['target_1']}), move SL to Cost, let remainder ride to T2 ({plan['target_2']})."
            )
...
backup_timestamp = file_path.with_name(
    f"auto_alerts.json.remediated.{datetime.now(IST).strftime('%Y%m%d_%H%M%S')}"
)
...
print(
    f"Successfully remediated {remediated_count} alerts and synchronized {file_path.name} & {bak_path.name}"
)
