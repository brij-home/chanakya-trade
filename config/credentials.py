...
─────────────────────
Secure credential management for chanakya-trade.
...
Keychain service name: "chanakya-trade"
Keys are stored as:   keyring.get_password("chanakya-trade", "KITE_API_KEY")
"""
...
KNOWN_CREDENTIALS: list[tuple[str, str, bool]] = [
    ...
    (
        "AI_PROVIDER",
        "AI Provider (gemini / groq / nvidia / deepseek / openrouter / anthropic / openai / ollama)",
        False,
    ),
    ...
    (
        "AI_DEEP_PROVIDER",
        "Provider for deep reasoning (gemini/nvidia/openrouter/anthropic/openai)",
        False,
    ),
    (
        "AI_DEEP_MODEL",
        "Deep model for reasoning/synthesis (e.g. gemini-3.7-flash, claude-opus-4-5)",
        False,
    ),
    (
        "AI_FAST_PROVIDER",
        "Provider for fast extraction if different from deep (gemini/groq/anthropic)",
        False,
    ),
    (
        "AI_FAST_MODEL",
        "Fast model for extraction/classification (e.g. gemini-3.7-flash, claude-haiku-3-5)",
        False,
    ),
    # ── Notifications ─────────────────────────────────────────
...
def _kr_get(key: str) -> Optional[str]:
    if os.environ.get("KEYRING_DISABLE") == "1" or os.environ.get("CI"):
        return None
    try:
        import keyring
