...
from web.schemas import (
    ...
    AlertAddRequest,
    AlertRemoveRequest,
    AutoAlertsListRequest,
    AlertPreferencesUpdateRequest,
    AutoAlertArchiveRequest,
    AutoAlertCleanupRequest,
    AutoAlertTestRequest,
    AutoAlertTargetTestRequest,
    AlertInvalidateRequest,
...
async def skill_options_chain(req: SymbolRequest):
    ...
    try:
        ...
        chain = await asyncio.to_thread(get_options_chain, req.symbol.upper(), None)
        return _ok(chain)
    except Exception as e:
        raise _err(str(e))
...
async def skill_flows():
    ...
    try:
        ...
        report = await asyncio.to_thread(get_flow_analysis)
        return _ok(report)
    except Exception as e:
        raise _err(str(e))
...
async def skill_analyze(req: AnalyzeRequest):
    ...
    try:
        ...
        report = await asyncio.to_thread(analyzer.analyze, sym, exch)
...
async def skill_auto_alerts_scan_now():
    ...
    try:
        ...
        new_alerts = await asyncio.to_thread(auto_alert_engine.scan_all_now)
        all_alerts = await asyncio.to_thread(auto_alert_engine.get_alerts, 50)
        return {
            "status": "ok",
            "data": {
